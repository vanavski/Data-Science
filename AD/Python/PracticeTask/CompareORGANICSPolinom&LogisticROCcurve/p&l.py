#TODO: need to know how change or throw out miss values, how to convert string type to float
...
#x=df[['DemAffl','DemAge','DemCluster','DemClusterGroup','DemGender','DemReg','DemTVReg','PromClass','PromSpend','PromTime','TargetBuy','TargetAmt']]
#median=df[['DemAffl','DemAge','DemCluster','PromTime','TargetBuy','TargetAmt']].median()

list = ['DemAffl','DemAge','DemCluster','PromTime','TargetBuy','TargetAmt']
for l in list:
    median = df[l].median()
    df[l].fillna(median, inplace=True)
#x = df[['DemAffl','DemAge','DemCluster','PromTime','TargetBuy','TargetAmt']].fillna(median, inplace=True)
y=df['ID']
x = df[['DemAffl','DemAge','DemCluster','PromTime','TargetBuy','TargetAmt']]
#y=y.astype('int')
print(x.head(140))
...
# It is important to t5rain the ensemble of trees on a different subset
